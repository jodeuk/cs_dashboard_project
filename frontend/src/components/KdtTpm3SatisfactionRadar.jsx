...
import { kdtTpm3DetailSeparated } from "../constants/kdt_tpm3_satisfaction";

export default function KdtTpm3SatisfactionRadar({ cardWidth = 700, cardHeight = 700 }) {
  // 차트 자체는 600x600으로 고정
  const chartSize = 600;
  const margin = 80;
  ...
  const radarData = useMemo(() => {
    const { theory, project } = kdtTpm3DetailSeparated;
    ...
    // 오피스아워 코치 (프로젝트주간에만)
    Object.entries(project.officeHourCoach).forEach(([key, values]) => {
      data.push({
        label: key,
        value: calcAvg(values),
        category: "오피스아워코치"
  ...
  // 동심원들 (점수 가이드라인)
  const circles = [3.0, 3.5, 4.0, 4.5, 5.0];
  ...
  const getCategoryColor = (category) => {
    switch(category) {
      ...
      case "오피스아워코치": return "#51cf66"; // 초록색
    ...
    <div style={{ width: "100%", overflowX: "auto", marginBottom: 24 }}>
      <svg width={cardWidth} height={cardHeight} style={{ background: "#fff", borderRadius: 8, boxShadow: "0 1px 6px rgba(0,0,0,0.08)" }}>
        ...
        <g transform={`translate(${(cardWidth - chartSize) / 2}, ${(cardHeight - chartSize) / 2 + 50})`}>
          ...
          {axes.map((axis, i) => {
            ...
            const shortLabel = axis.label
              ...
              .replace("커리큘럼 난이도 적절성", "난이도")
              .replace("퀴즈/실습 자료", "실습자료")
              .replace("목표한 결과물 달성", "결과물달성")
              .replace("스크럼 시간 효율성", "스크럼효율")
              .replace("개발 역량 향상", "개발역량")
              .replace("프로젝트 가이드 충분성", "가이드충분")
              .replace("메인코치 전반적", "코치전반")
              .replace("메인코치 적극성과 참여도", "코치참여")
              .replace("질문 응답 적극성", "질의응답")
              .replace("프로젝트 도움 충분성", "도움충분")
              .replace("강의 내용 이해하기 쉬운 설명", "강의이해")
              .replace("충분한 상호작용 및 소통", "상호작용")
              .replace("강의 준비 성실성", "강의준비");
        ...
        <text x={cardWidth / 2} y={30} fontSize="14" fontWeight="600" fill="#212529" textAnchor="middle">
          [KDT TPM3기] 세부 항목 레이더 차트
        </text>

        {/* 범례 - 카드 전체 기준으로 위치 */}
        <g transform={`translate(${cardWidth - 200}, 20)`}>
          <rect width="190" height="100" rx="6" fill="#f8f9fa" stroke="#dee2e6" />
          ...
          {[
            ...
            { label: "오피스아워 코치 만족도", color: "#51cf66" }
          ].map((item, i) => (
